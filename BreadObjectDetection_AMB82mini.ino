...
void setup() {
  ...
  OSD.configTextSize(CHANNEL, 50, 100);
  OSD.begin();
}
...
void loop() {
  ...
  if (ObjDet.getResultCount() > 0) {
    ...
    for (uint32_t i = 0; i < ObjDet.getResultCount(); i++) {
      ...
      //  檢查是否要篩選=1=True(0=False)
      if (itemList[obj_type].filter) {  // check if item should be ignored
        //item=這個辨識結果的物件（編號,位置,信任度...）
        ...
        //計算總價格
        //1. 將Total_Amonut加上物件的單價Price
        Total_Amount = Total_Amount + itemList[obj_type].Price;        

      }
    }
    //2. 將結果顯示在左上角
    String AmountText="Amonut:" + String(Total_Amount);
    //                                                       字串轉字元陣列String->char []    
    OSD.drawText(CHANNEL, 200, 200 , AmountText.c_str() , OSD_COLOR_RED);
  }
  OSD.update(CHANNEL);
